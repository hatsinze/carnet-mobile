  ...
  return (
    <View>
      <View style={styles.header}>
        ...
        <Text style={styles.monthLabel}>{currentMonth.toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}</Text>
        <Pressable onPress={goNext} hitSlop={10}><Ionicons name="chevron-forward" size={20} color={colors.encre} /></Pressable>
      </View>

      <View style={styles.weekdayRow}>
        {WEEKDAYS.map((w, i) => <Text key={i} style={styles.weekdayLabel}>{w}</Text>)}
      </View>
...
const styles = StyleSheet.create({
  ...
  monthLabel: { fontFamily: fonts.displaySemiBold, fontSize: 17, color: colors.ardoise, textTransform: 'capitalize' },
  weekdayRow: { flexDirection: 'row', marginBottom: spacing.xs },
  weekdayLabel: { flex: 1, textAlign: 'center', fontFamily: fonts.bodyMedium, fontSize: 11, color: colors.ardoiseMuted },
  ...
  dayNumberWrapSelected: { backgroundColor: colors.encre },
  dayNumber: { fontFamily: fonts.bodyMedium, fontSize: 13, color: colors.ardoise },
  dayNumberToday: { fontFamily: fonts.bodyBold, color: colors.encre },
  dayNumberSelected: { color: colors.blanc, fontFamily: fonts.bodyBold },
