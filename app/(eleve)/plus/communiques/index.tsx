...
export default function CommuniquesArchiveScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.brume }]}>
      ...
      <ScreenHeader 
        title="Communiqués" 
        fallbackRoute="/(eleve)/plus" 
        right={<Text style={{ fontFamily: fonts.bodyMedium, fontSize: 12, color: colors.ardoiseMuted }}>{allItems.length}</Text>} 
      />
      ...
      {isLoading ? (
        ...
        <SectionList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <Pressable onPress={() => router.push(`/(eleve)/plus/communiques/${item.id}`)}>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  searchBar: { flexDirection: 'row', alignItems: 'center', borderRadius: radius.md, borderWidth: 1, marginHorizontal: spacing.lg, marginTop: spacing.md, paddingHorizontal: spacing.md, gap: spacing.sm },
