...
export default function MessagesScreen() {
  ...
  return (
    <View style={styles.container}>
      ...
      {!isLoading && items.length === 0 && <EmptyState message="Aucune conversation pour le moment." />}
        ...
        renderItem={({ item }) => (
          <Pressable onPress={() => router.push(`/(parent)/messages/${item.id}`)}>
            <Card style={styles.card}>
              ...
              {item.dernier_message && <Text style={styles.excerpt} numberOfLines={1}>{item.dernier_message.contenu}</Text>}
            </Card>
          </Pressable>
        )}
      />

      <Pressable style={styles.fab} onPress={() => router.push('/(parent)/messages/new')}>
...
const styles = StyleSheet.create({
  ...
  fab: { position: 'absolute', right: spacing.lg, bottom: spacing.lg, width: 56, height: 56, borderRadius: radius.lg, backgroundColor: colors.encre, justifyContent: 'center', alignItems: 'center', elevation: 4, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 4 },
});