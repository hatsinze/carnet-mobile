import { View, Text, ScrollView, Pressable, StyleSheet } from 'react-native';
import { useLocalSearchParams, useRouter } from 'expo-router';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { Ionicons } from '@expo/vector-icons';
import { LoadingState } from '../../../src/components/LoadingState';
import { ErrorState } from '../../../src/components/ErrorState';
import { FadeInUp } from '../../../src/components/Motion';
import { useCommunique, useConfirmerPresence } from '../../../src/hooks/useCommuniques';
import { colors, fonts, radius, spacing } from '../../../src/theme/tokens';

export default function CommuniqueDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const communiqueId = Number(id);
  const { data, isLoading, isError, refetch } = useCommunique(communiqueId);
  const confirmerPresence = useConfirmerPresence(communiqueId);

  function goBack() {
    if (router.canGoBack()) router.back();
    else router.replace('/(parent)/communiques');
  }

  if (isLoading) return <LoadingState />;
  if (isError) return <ErrorState onRetry={refetch} />;

  if (!data) return null;

  // Format date for display
  const formattedDate = data.publie_le 
    ? new Date(data.publie_le.replace(' ', 'T')).toLocaleDateString('fr-FR', {
        day: 'numeric',
        month: 'long',
        year: 'numeric',
        hour: '2-digit',
        minute: '2-digit',
      })
    : '';

  return (
    <View style={styles.container}>
      {/* Simple Header - WhatsApp Style */}
      <View style={[styles.header, { paddingTop: insets.top + spacing.sm }]}>
        <Pressable onPress={goBack} hitSlop={10} style={styles.backButton}>
          <Ionicons name="chevron-back" size={24} color={colors.encre} />
          <Text style={styles.backLabel}>Retour</Text>
        </Pressable>
        <View style={styles.headerRight}>
          <View style={[styles.statusDot, { backgroundColor: data.lu ? colors.sauge : colors.or }]} />
          <Text style={styles.statusText}>{data.lu ? 'Lu' : 'Non lu'}</Text>
        </View>
      </View>

      <ScrollView 
        contentContainerStyle={styles.content}
        showsVerticalScrollIndicator={false}
      >
        <FadeInUp delay={80}>
          {/* Title - Twitter Style */}
          <Text style={styles.title}>{data.titre}</Text>

          {/* Meta - Twitter Style */}
          <View style={styles.metaRow}>
            <View style={styles.metaItem}>
              <Ionicons name="person-outline" size={14} color={colors.ardoiseMuted} />
              <Text style={styles.metaText}>{data.auteur || 'Administration'}</Text>
            </View>
            <View style={styles.metaDot} />
            <View style={styles.metaItem}>
              <Ionicons name="time-outline" size={14} color={colors.ardoiseMuted} />
              <Text style={styles.metaText}>{formattedDate}</Text>
            </View>
          </View>

          {/* Body - WhatsApp Style */}
          <View style={styles.bodyContainer}>
            <Text style={styles.body}>{data.contenu}</Text>
          </View>

          {/* Meeting Section - WhatsApp Style */}
          {data.est_reunion && (
            <FadeInUp delay={120}>
              <View style={styles.meetingCard}>
                <View style={styles.meetingHeader}>
                  <Ionicons name="calendar-outline" size={18} color={colors.or} />
                  <Text style={styles.meetingBadgeText}>Réunion</Text>
                </View>

                {data.date_heure_reunion && (
                  <View style={styles.meetingRow}>
                    <Ionicons name="time-outline" size={16} color={colors.ardoiseMuted} />
                    <Text style={styles.meetingText}>
                      {new Date(data.date_heure_reunion.replace(' ', 'T')).toLocaleDateString('fr-FR', {
                        day: 'numeric',
                        month: 'long',
                        year: 'numeric',
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </Text>
                  </View>
                )}
                {data.lieu && (
                  <View style={styles.meetingRow}>
                    <Ionicons name="location-outline" size={16} color={colors.ardoiseMuted} />
                    <Text style={styles.meetingText}>{data.lieu}</Text>
                  </View>
                )}

                <View style={styles.meetingDivider} />

                {data.confirmation === null ? (
                  <View style={styles.confirmRow}>
                    <Pressable
                      style={[styles.confirmButton, styles.confirmButtonPrimary]}
                      onPress={() => confirmerPresence.mutate('oui')}
                      disabled={confirmerPresence.isPending}
                    >
                      <Ionicons name="checkmark-outline" size={18} color={colors.blanc} />
                      <Text style={styles.confirmButtonPrimaryText}>Confirmer</Text>
                    </Pressable>
                    <Pressable
                      style={[styles.confirmButton, styles.confirmButtonSecondary]}
                      onPress={() => confirmerPresence.mutate('non')}
                      disabled={confirmerPresence.isPending}
                    >
                      <Ionicons name="close-outline" size={18} color={colors.brique} />
                      <Text style={styles.confirmButtonSecondaryText}>Décliner</Text>
                    </Pressable>
                  </View>
                ) : (
                  <View style={[styles.confirmedBanner, { 
                    backgroundColor: data.confirmation === 'oui' ? '#E8F5E9' : '#FFEBEE' 
                  }]}>
                    <Ionicons
                      name={data.confirmation === 'oui' ? 'checkmark-circle' : 'close-circle'}
                      size={20}
                      color={data.confirmation === 'oui' ? colors.sauge : colors.brique}
                    />
                    <Text style={[styles.confirmedText, { 
                      color: data.confirmation === 'oui' ? colors.sauge : colors.brique 
                    }]}>
                      {data.confirmation === 'oui' ? 'Présence confirmée' : 'Vous avez décliné cette réunion'}
                    </Text>
                  </View>
                )}
              </View>
            </FadeInUp>
          )}

          <View style={styles.bottomSpacer} />
        </FadeInUp>
      </ScrollView>
    </View>
  );
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#FFFFFF',
  },

  // Simple Header - WhatsApp Style
  header: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    paddingHorizontal: spacing.lg,
    paddingBottom: spacing.sm,
    backgroundColor: '#FFFFFF',
    borderBottomWidth: 1,
    borderBottomColor: '#F0F0F0',
  },
  backButton: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
  },
  backLabel: {
    fontFamily: fonts.bodyMedium,
    fontSize: 15,
    color: colors.encre,
  },
  headerRight: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
  },
  statusDot: {
    width: 8,
    height: 8,
    borderRadius: 4,
  },
  statusText: {
    fontFamily: fonts.bodyMedium,
    fontSize: 12,
    color: colors.ardoiseMuted,
  },

  content: {
    paddingHorizontal: spacing.lg,
    paddingTop: spacing.lg,
    paddingBottom: spacing.xxxl,
  },

  // Title - Twitter Style
  title: {
    fontFamily: fonts.displayBold,
    fontSize: 24,
    color: colors.ardoise,
    lineHeight: 30,
    letterSpacing: -0.5,
    marginBottom: spacing.md,
  },

  // Meta - Twitter Style
  metaRow: {
    flexDirection: 'row',
    alignItems: 'center',
    flexWrap: 'wrap',
    gap: 6,
    marginBottom: spacing.lg,
  },
  metaItem: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
  },
  metaText: {
    fontFamily: fonts.body,
    fontSize: 13,
    color: colors.ardoiseMuted,
  },
  metaDot: {
    width: 3,
    height: 3,
    borderRadius: 1.5,
    backgroundColor: colors.ardoiseMuted,
    opacity: 0.5,
  },

  // Body - WhatsApp Style
  bodyContainer: {
    backgroundColor: '#F8F9FA',
    borderRadius: radius.md,
    padding: spacing.lg,
    marginBottom: spacing.lg,
  },
  body: {
    fontFamily: fonts.body,
    fontSize: 16,
    color: colors.ardoise,
    lineHeight: 26,
  },

  // Meeting Card - WhatsApp Style
  meetingCard: {
    backgroundColor: '#F8F9FA',
    borderRadius: radius.md,
    padding: spacing.lg,
    marginTop: spacing.sm,
  },
  meetingHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
    marginBottom: spacing.md,
  },
  meetingBadgeText: {
    fontFamily: fonts.bodySemiBold,
    fontSize: 14,
    color: colors.or,
  },
  meetingRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing.sm,
    marginTop: spacing.sm,
  },
  meetingText: {
    fontFamily: fonts.body,
    fontSize: 14,
    color: colors.ardoise,
    flex: 1,
  },
  meetingDivider: {
    height: 1,
    backgroundColor: '#E8ECF0',
    marginVertical: spacing.md,
  },

  // Confirm Buttons - WhatsApp Style
  confirmRow: {
    flexDirection: 'row',
    gap: spacing.sm,
  },
  confirmButton: {
    flex: 1,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    gap: 6,
    borderRadius: radius.md,
    paddingVertical: spacing.md,
  },
  confirmButtonPrimary: {
    backgroundColor: colors.encre,
  },
  confirmButtonPrimaryText: {
    fontFamily: fonts.bodySemiBold,
    fontSize: 14,
    color: colors.blanc,
  },
  confirmButtonSecondary: {
    borderWidth: 1.5,
    borderColor: colors.brique,
    backgroundColor: '#FFFFFF',
  },
  confirmButtonSecondaryText: {
    fontFamily: fonts.bodySemiBold,
    fontSize: 14,
    color: colors.brique,
  },

  // Confirmed Banner - WhatsApp Style
  confirmedBanner: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing.sm,
    borderRadius: radius.md,
    padding: spacing.md,
  },
  confirmedText: {
    fontFamily: fonts.bodySemiBold,
    fontSize: 14,
  },

  bottomSpacer: {
    height: spacing.xl,
  },
});