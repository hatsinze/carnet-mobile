...
function EventRow({ event, muted }: { event: EvenementCalendrier; muted?: boolean }) {
    ...
    >
      ...
      {viewMode === 'mois' ? (
        <FadeInUp key="mois">
          ...
          <Text style={styles.sectionTitle}>{selectedDate.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}</Text>
          {dayEvents.length === 0 ? (
            <View style={styles.emptyDay}>
              <Ionicons name="calendar-clear-outline" size={20} color={colors.ardoiseMuted} />
              <Text style={styles.emptyDayText}>Aucun événement ce jour-là</Text>
            </View>
          ) : (
            dayEvents.map((ev, i) => (
              <View key={ev.id}>
                <EventRow event={ev} />
                {i < dayEvents.length - 1 && <View style={styles.divider} />}
              </View>
            ))
          )}
        </FadeInUp>
        ...
        <FadeInUp key="liste">
          {upcoming.length === 0 && past.length === 0 && (
            <View style={styles.emptyDay}>
              ...
              <Text style={styles.emptyDayText}>Aucun événement à venir</Text>
            </View>
          )}
          {upcoming.length > 0 && (
            <>
          ...
          {past.length > 0 && (
            <>
              <Text style={[styles.sectionTitle, { marginTop: spacing.xl }]}>Récemment passés</Text>
              {past.map((ev, i) => (
                <View key={ev.id}>
                  <EventRow event={ev} muted />
...
const styles = StyleSheet.create({
  ...
  calendarBox: { borderWidth: 1, borderColor: colors.ligne, borderRadius: radius.lg, padding: spacing.lg, marginBottom: spacing.xl },
  sectionTitle: { fontFamily: fonts.displaySemiBold, fontSize: 16, color: colors.ardoise, marginBottom: spacing.md, textTransform: 'capitalize' },
  emptyDay: { alignItems: 'center', paddingVertical: spacing.xxl, gap: spacing.sm },
  emptyDayText: { fontFamily: fonts.body, fontSize: 13, color: colors.ardoiseMuted },
  ...
  eventTitle: { fontFamily: fonts.bodySemiBold, fontSize: 14, color: colors.ardoise },
  eventDate: { fontFamily: fonts.body, fontSize: 12, color: colors.ardoiseMuted, marginTop: 2 },
